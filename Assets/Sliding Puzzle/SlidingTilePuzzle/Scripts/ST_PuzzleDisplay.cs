...
public class ST_PuzzleDisplay : MonoBehaviour
{
	...
	// to initilaize the puzzle to play
	public void Initpuzzle(Texture texture, int puzzleId)
	{
		PlayMenuSFX();

		audioManager.PlayMusic(audioManager.puzzleBGM);

		LoadLevelData();
		puzzle_id = puzzleId;

		var runningtime = exitpuzzlebutton.GetComponentInChildren<TextMeshProUGUI>();
		runningtime.text = "00.00s";


		if (texture != null)
		{
			PuzzleImage = texture;
			Debug.Log("PuzzleImage initialized with the new texture!");
		}
		else
		{
		...
		exitpuzzlebutton.SetActive(true);
		elapsedTime = 0f;  // Reset timer
		isTimerRunning = false;  // Timer starts only on first tile click
	...
	void Update()
	{
		...
		if (isTimerRunning)
		{
			...
			var runningtime = exitpuzzlebutton.GetComponentInChildren<TextMeshProUGUI>();
			runningtime.text = $"{elapsedTime:00.00}s";
		}
	}
	...
	public IEnumerator CheckForComplete()
	{
		...
		if (Complete)
		{
			...
			var levell = levelDataList.Find(PuzzleLevelData => PuzzleLevelData.Id == puzzle_id);
			if (levell.bestTime > elapsedTime)
			{
				levell.bestTime = elapsedTime;
			}
			levell.isCompleted = 1;
			SaveLevelData();
	...
	private void SaveGameData()
	{
		InventoryItem targetItem = InventoryManager.Instance.inventory.items.Find(
			...
			{
				return item.Id == puzzle_id;
			}
		);
		int currIndex = InventoryManager.Instance.inventory.items.IndexOf(targetItem);

		if (Save.instance.lastUnlockedSlidingTile < 5 && (currIndex + 1) == Save.instance.lastUnlockedSlidingTile)
		{
			Save.instance.lastUnlockedSlidingTile += 1;
		}
	...
	private void CreatePuzzleTiles()
	{
		...
		for (int j = Height - 1; j >= 0; j--)
		{
			for (int i = 0; i < Width; i++)
			{
				...
				// Shader urpShader = Shader.Find("Standard");
